...
SSL* create_tls_connection(const std::string& host, int port, int& socket_fd, const char* remote_ip, const char* local_ip) {
    ...
    // Set minimum TLS version to 1.2 (many servers require this)
    // SSL_CTX_set_min_proto_version(ctx, TLS1_2_VERSION);
    
    // Enable default cipher list
    ...
    // CRITICAL: Set SNI (Server Name Indication) - this is required by most modern servers
    // This tells the server which hostname we're trying to connect to
    if (SSL_set_tlsext_host_name(ssl, host.c_str()) != 1) {
        std::cerr << "Error: Failed to set SNI hostname." << std::endl;
        ERR_print_errors_fp(stderr);
        SSL_free(ssl);
        close(socket_fd);
        SSL_CTX_free(ctx);
    ...
    if (SSL_connect(ssl) <= 0) {
        std::cerr << "Error: TLS connect failed." << std::endl;
        ERR_print_errors_fp(stderr);
        SSL_free(ssl);
        close(socket_fd);
        return nullptr;
